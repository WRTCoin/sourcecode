...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn, CWallet* pwallet, bool fProofOfStake)
{
...

    {
        ...
        LogPrint("worthless", "CreateNewBlock(): total size %u\n", nBlockSize);
...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("worthlessMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock)) {
        ...
        return error("worthlessMiner : ProcessNewBlock, block not accepted");
    }
...
{
    LogPrintf("worthlessMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("worthless-miner");
    ...
    while (fGenerateBitcoins || fProofOfStake) {
        ...
        LogPrint("worthless", "Running worthlessMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
