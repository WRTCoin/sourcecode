# worthless RPC Commands and arguments
...
The configuration file is not automatically created; you can create it using your favorite plain-text editor. By default, worthless-qt (or worthlessd) will look for a file named worthless.conf in the worthless data directory, but both the data directory and the configuration file path may be changed using the -datadir and -conf command-line arguments.

| Platform	   |Path to data folder           | Typical path to configuration file                                                 |
|------------|--------------------------------|------------------------------------------------------------------------------------|
| Linux	     | ~/	                            | /home/username/.worthless/worthless.conf                                               |
| macOS      | ~/Library/Application Support/	| /Users/username/Library/Application Support/Eletra/worthless.conf                    |
| Windows	   | %APPDATA%	                    | (Vista-10) C:\Users\username\AppData\Roaming\worthless\worthless.conf                  |
|            |                                | (2000-XP) C:\Documents and Settings\username\Application Data\worthless\worthless.conf |

Note: if running worthless in testnet mode, the sub-folder testnet will be appended to the data directory automatically. Command line arguments

## These commands are accurate as of worthless Core version 2.0.0:
...
Start worthless Core Daemon with option:
<table>
<tr> <td>worthlessd</td><td>[options]</td></tr>
</table>
...
<tr> <td>--conf=<file></td><td>Specify configuration file (default: worthless.conf)</td></tr>
...
<tr> <td>--pid=<file></td><td>Specify pid file (default: worthlessd.pid)</td></tr>
...
<tr> <td>--reindexmoneysupply</td><td>Reindex the worthless and zECA money supply statistics on startup</td></tr>
...
<tr> <td>--paytxfee=<amt></td><td>Fee (in worthless/kB) to add to transactions you send (default: 0.00)
...
<tr> <td>--minrelaytxfee=<amt></td><td>Fees (in worthless/Kb) smaller than this are considered zero fee for relaying (default: 0.0001)
...
<tr> <td>-litemode=<n></td><td>Disable all worthless specific functionality (Masternodes, Zerocoin, wiftX, Budgeting) (0-1, default: 0)
</table>
...
<tr> <td>--ecastake=<n></td><td>Enable or disable staking functionality for worthless inputs (0-1, default: 1)
...
This documentation lists all available RPC commands as of worthless version 2.0.0, and limited documentation on what each command does. For full documentation of arguments, results and examples, type help ( “command” ) to view full details at the console. You can enter commands either from Tools > Debug console in the QT wallet, or using worthless-cli
...
#### debug ( 0 | 1 | addrman | alert | bench | coindb | db | lock | rand | rpc | selectcoins | mempool | mempoolrej | net | proxy | prune | http | libevent | worthless )
...
#### preparebudget "proposal-name" "url" payment-count block-start "worthless-address" monthy-payment
...
#### submitbudget "proposal-name" "url" payment-count block-start "worthless-address" monthy-payment "fee-tx"
...
PoW Only: Return if the server is set to generate coins or not. The default is false. It is set with the command line argument -gen (or worthless.conf setting gen) It can also be set with the setgenerate call.
...
#### validateaddress “worthlessaddress”

Return information about the given worthless address.

#### verifymessage “worthlessaddress” “signature” “message”
...
#### bip38decrypt "worthlessaddress" "passphrase"

Decrypts and then imports password protected private key.

#### bip38encrypt "worthlessaddress" "passphrase"

Encrypts a private key corresponding to 'worthlessaddress'.

#### dumpprivkey “worthlessaddress”

Reveals the private key corresponding to ‘worthlessaddress’. Then the importprivkey can be used with this output
...
DEPRECATED. Returns the current worthless address for receiving payments to this account.
...
Returns a new worthless address for receiving payments. If ‘account’ is specified (DEPRECATED), it is added to the address book so payments received with the address will be credited to ‘account’.

#### getrawchangeaddress

Returns a new worthless address, for receiving change. This is for use with raw transactions, NOT normal use.

#### getreceivedbyaccount “account” ( minconf addlockconf )

DEPRECATED. Returns the total amount received by addresses with <account> in transactions with specified minimum number of confirmations.

#### getreceivedbyaddress “worthlessaddress” ( minconf addlockconf )

Returns the total amount received by the given worthlessaddress in transactions with specified minimum number of confirmations.
...
#### importprivkey “worthlessprivkey” ( “label” rescan )
...
#### sendfrom “fromaccount” “toworthlessaddress” amount ( minconf addlockconf “comment” “comment-to” )

DEPRECATED (use sendtoaddress). Sent an amount from an account to a worthless address.
...
#### sendtoaddress “worthlessaddress” amount ( “comment” “comment-to” subtractfeefromamount use_is use_ps )
...
Stores the wallet decryption key in memory for ‘timeout’ seconds. This is needed prior to performing transactions related to private keys such as sending worthlesss
...
Usage:
  worthless-tx [options] <hex-tx> [commands]  Update hex-encoded worthless transaction
  worthless-tx [options] -create [commands]   Create hex-encoded worthless transaction
